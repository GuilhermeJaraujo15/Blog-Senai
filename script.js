...
if (languageToggle) {
  languageToggle.addEventListener("click", () => {
    const nextLocale = window.i18n?.currentLocale?.() === "en" ? "pt-BR" : "en";
    window.i18n?.setLanguage?.(nextLocale);
  });
}
...
addComunidade.addEventListener("click", () => {
  ...
  if (!emailValor.endsWith("@aluno.senai.br")) {
    alert(window.i18n?.t?.("alerts.invalidEmail") || "Apenas e-mails institucionais (@aluno.senai.br) podem enviar comentários.");
    return;
  }

  if (nome.value.trim() === "") {
    alert(window.i18n?.t?.("alerts.nameRequired") || "Preencha o nome");
    return;
  }

  if (dataInput.value.trim() === "") {
    alert(window.i18n?.t?.("alerts.dateRequired") || "Preencha a data");
    return;
  }

  if (texto.value.trim() === "") {
    alert(window.i18n?.t?.("alerts.textRequired") || "Preencha o texto");
    return;
  }

  if (tag.value.trim() === "") {
    alert(window.i18n?.t?.("alerts.tagRequired") || "Preencha a tag");
    return;
  }
  ...
  const dataFormatada = window.i18n?.formatDate?.(valorData) || valorData;
